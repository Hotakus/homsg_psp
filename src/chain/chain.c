...
//static chain_node_t *chain_node_step(chain_node_t *node, unsigned char steps, bool forward);
//
//static void chain_poll(chain_t *chain, bool forward);
//static chain_node_t *node_create(const char *name, void *data);
//static void chain_node_destroy(chain_node_t *node);
//static void chain_node_connect(chain_node_t *dst_node, chain_node_t *src_node, bool front);
//
...
//static chain_node_t *chain_get_loop_end_node(chain_t *chain);
//static chain_node_t *chain_determine_junction_node(chain_t *chain, chain_node_t *collision_node);
 ...
 * @return 一个新节点
 */
chain_node_t *chain_node_create(const char *name, void *data) {
 ...
 * @param node
 */
void chain_node_destroy(chain_node_t *node) {
    if (node == NULL) {
#if DEBUG == 1
...
chain_t *chain_create(char *desc) {
    ...
    chain->head = chain_node_create("head", NULL);
    chain->tail = chain_node_create("tail", NULL);
...
#if (CHAIN_USE_BUILTIN == 1)
    // default function
    chain->poll = chain_poll;
    chain->node_new = chain_node_create;
    chain->node_del = chain_node_destroy;
    chain->node_conn = chain_node_connect;
    ...
    chain->get_loop_end = chain_get_loop_end_node;
#endif
...
void chain_destroy(chain_t *chain) {
    ...
    chain_node_destroy(chain->head);
    chain_node_destroy(chain->tail);
...
void chain_poll(chain_t *chain, bool forward) {
    ...
    if (forward) {
        ...
        while (probe != chain->tail->next_node) {
            printf("%s \t %zu \t 0x%x \n", probe->name, probe->id, probe->data);
            probe = probe->next_node;
        }
    } else {
 ...
 * @param name node的名字
 */
chain_node_t *chain_remove_node(chain_t *chain, const char *name, chain_node_t *_node, bool destroy) {
    ...
    chain_node_connect(p_node, n_node, false);
    chain->length -= 1;

    if (destroy) {
        chain_node_destroy(node);
        return NULL;
    }
 ...
 */
void chain_node_connect(chain_node_t *dst_node, chain_node_t *src_node, bool front) {
    if (front) {
 ...
 * @return 步进后节点指针
 */
chain_node_t *chain_node_step(chain_node_t *node, unsigned char steps, bool forward) {
...
bool chain_has_loop(chain_t *chain, bool detach) {
    ...
    while (fast->next_node != NULL) {
        fast = chain_node_step(fast, fast_step, true);
        slow = chain_node_step(slow, slow_step, true);

        if (fast == slow) {
            // TODO: determine junction
            chain_node_t *junction = chain_determine_junction_node(chain, fast);
            ...
            if (chain->loop_info == NULL) {
                ...
                chain->loop_info->end_node = chain_get_loop_end_node(chain);
            }
...
int chain_loop_length(chain_node_t *collision_node) {
    ...
    while (fast->next_node != NULL) {
        ...
        fast = chain_node_step(fast, 2, true);
        slow = chain_node_step(slow, 1, true);
 ...
 * @return 连接点
 */
chain_node_t *chain_determine_junction_node(chain_t *chain, chain_node_t *collision_node) {
    chain_node_t *head = chain->head;
    chain_node_t *collision = collision_node;

    while (head->next_node != NULL) {
        head = chain_node_step(head, 1, true);
        collision = chain_node_step(collision, 1, true);
        if (head == collision) {
            return head;
        }
    }
